...
namespace FM.FrameWork.Database
{
    public abstract class AbpDapperRepositoryBase<TEntity, TPrimaryKey> : IDapperRepository<TEntity, TPrimaryKey>, IRepository, ITransientDependency where TEntity : class, IEntity<TPrimaryKey>
    {
        ...
        protected static Expression<Func<TEntity, bool>> CreateEqualityExpressionForId(TPrimaryKey id)
        {
            ...
            ConstantExpression right = Expression.Constant(Convert.ChangeType(id, typeof(TPrimaryKey)), typeof(TPrimaryKey));
            return Expression.Lambda<Func<TEntity, bool>>(Expression.Equal(Expression.PropertyOrField(parameterExpression, "Id"), right), new ParameterExpression[1] { parameterExpression });
        }
    }
}
